...
def generate_clauses(n,m,c,time_list,adj,ip1,ip2,X,S,A, peak):
    ...
    # SM-T: if i and j share a station and i starts at t, then j must
    # finish before i starts or start after i finishes.  T is interpreted
    # as an extended cumulative variable: false below zero and true beyond
    # j's latest-start prefix.  In particular, an out-of-range true literal
    # makes the whole clause tautological; it must not simply be dropped.
    for i in range(n-1):
        for j in range(i+1,n):
            last_j = c - time_list[j]
            for t in range(c - time_list[i] + 1):
                ...
                if t_left > max_t_index:
                    # Extended T[j,t_left] is true: the clause is a tautology.
                    continue
                if t_left >= 0:
                    clause.append(get_var("T", j, t_left))
                t_right = t + time_list[i] - 1
                if t_right < 0:
                    # Extended -T[j,t_right] is true: the clause is a tautology.
                    continue
                if t_right <= max_t_index:
